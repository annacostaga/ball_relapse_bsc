...
# STEP 1:
We have 1.303.771 significant interactions (chicago >=5 in total), and 200.343 nodes: 21.299 baits, and 179.044 OE.
...
png("/home/acost1/BALL_project/results/gnn/figures/interactions_nodes_step1.png", width = 50, height = 30, res = 200, units = "cm")
...
See the distributions of the bin lengths for Baits and other ends
```{r}
...
Now, we have 632.352 significant interactions (chicago >=5 in total), and 114.441 nodes: 18.740 baits, and 95.701 OE.
...
png("/home/acost1/BALL_project/results/gnn/figures/interactions_nodes_step2.png", width = 50, height = 30, res = 200, units = "cm")
ggarrange(plot_interactions, plot_nodes, ncol = 2)
dev.off()
```
...
```{r}
png("/home/acost1/BALL_project/results/gnn/figures/nodes_step2.png", width = 50, height = 30, res = 200, units = "cm")
...
bait2_id[!bait2_id %in% bait1_id] %>% length() # only region 1: 554 baits


length(unique(c(bait1_id, bait2_id))) # 18.740

inter_baits <- rbind( bait1, bait2) %>%
                filter(!duplicated(.)) # only 18740 regions
...
# NOW n = 23.781 ---> unique ensembl id = 23.779, unique symbol = 21.191

# remove genes that are not annotated in chr 1:23, X, Y (n = 21281) --> unique = 21224 (removed n = 2555, unique = 0)
# NOW remove genes that are not annotated in chr 1:23, X, Y (n = 21247) --> unique = 21190 (removed n = 2534, unique = 1)
...
# NOW remove genes that have hgnc_symbol duplicated (n = 21133) --> unique = 21133  (removed n = 114, unique = 57)
...
#- Couldn't find 407 gene ensembl id 
...
```{r}
# Select only the baits with know ensembl id and expression (they come from inter_baits object)

aux <- inter_annotated_gene
...
inter_annotated_gene <- filter(inter_annotated_gene, !is.na(expression) ) # n = 497.157 => 626.087
...
filter(inter_annotated_gene, (is.na(expression.y) & node.class2 == "B") )
...
png("/home/acost1/BALL_project/results/gnn/figures/interactions_nodes_step3.png", width = 50, height = 30, res = 200, units = "cm")
...
```{r}
png("/home/acost1/BALL_project/results/gnn/figures/nodes_step3.png", width = 50, height = 30, res = 200, units = "cm")
...
table(chromhmm$cell_type)


chromhmm$cell_type <- factor(chromhmm$cell_type, 
                                       levels = c("HSC", "PrePro-B", "Pro-B", "Pre-B", "Trans-B", "nB", "GCB", "Mem-B", "Plasma"),
                                        labels = c("HSC", "PreProB", "ProB", "PreB", "immtransB", "nB1Mnew", "GCB", "memB", "PC"))
```
...
# Number of Baits:
length(unique(inter_baits$id)) 

# Numer of OEs
length(unique(inter_OE$id))
...
png("/home/acost1/BALL_project/results/gnn/figures/interactions_nodes_step4.png", width = 50, height = 30, res = 200, units = "cm")
...
```{r}
png("/home/acost1/BALL_project/results/gnn/figures/nodes_step4.png", width = 50, height = 30, res = 200, units = "cm")
